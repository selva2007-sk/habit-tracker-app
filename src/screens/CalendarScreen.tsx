...
const CalendarScreen: React.FC = () => {
  ...
  const currentMonthDays = calendarDays.filter(day => isSameMonth(day, monthStart));
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="glass-card p-4">
            ...
            <p className="text-2xl font-bold mt-1 text-[var(--accent-color)]">
              {habits.reduce((acc, h) => acc + h.completedDates.filter(d => isSameMonth(parseISO(d), currentMonth)).length, 0)}
            </p>
          </div>
          <div className="glass-card p-4">
            <p className="text-[var(--text-secondary)] text-[10px] uppercase tracking-wider font-bold">Avg Progress</p>
            <p className="text-2xl font-bold mt-1 text-[var(--accent-color)]">
              {Math.round(currentMonthDays.reduce((acc, day) => acc + (getCompletionForDay(day).length / (habits.length || 1)), 0) / Math.max(1, currentMonthDays.length) * 100)}%
            </p>
          </div>
        </div>
      </motion.div>
